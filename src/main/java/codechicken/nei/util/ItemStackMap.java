...
public class ItemStackMap<T> {
    ...
    public class DetailMap {
        ...
        public T get(ItemStack key) {
            ...
            if (metaMap != null) {
                T ret = metaMap.get(new StackMetaKey(key));
        ...
        public T remove(ItemStack key) {
            try {
                switch (getKeyType(actualDamage(key), key.getTagCompound())) {
                    case 0:
                        return metaMap != null ? metaMap.remove(new StackMetaKey(key)) : defaultReturn;
                    case 1:
                        return tagMap != null ? tagMap.remove(key.getTagCompound()) : defaultReturn;
                    case 2:
                        return damageMap != null ? damageMap.remove(actualDamage(key)) : defaultReturn;
                    case 3:
                        T ret = wildcard;
                        wildcard = null;
                        hasWildcard = false;
        ...
        private void updateSize() {
            int newSize = (hasWildcard ? 1 : 0) + (metaMap != null ? metaMap.size() : 0) + (tagMap != null ? tagMap.size() : 0) + (damageMap != null ? damageMap.size() : 0);
    ...
    public ItemStackMap(T defaultReturn) {
    ...
    public T get(ItemStack key) {
        ...
        return map == null ? defaultReturn : map.get(key);
    }
    ...
    public T remove(ItemStack key) {
        ...
        DetailMap map = itemMap.get(key.getItem());
        return map == null ? defaultReturn : map.remove(key);
    }
