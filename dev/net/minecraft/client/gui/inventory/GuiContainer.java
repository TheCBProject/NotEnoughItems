...
public abstract class GuiContainer extends GuiScreen
{
    ...
    protected void mouseReleased(int mouseX, int mouseY, int state)
    {
        ...
        if (this.doubleClick && slot != null && state == 0 && this.inventorySlots.canMergeSlot((ItemStack)null, slot))
        {
            if (isShiftKeyDown())
            {
                if (slot != null && slot.inventory != null && this.shiftClickedSlot != null)
                {
                    for (Slot slot2 : this.inventorySlots.inventorySlots)
                    {
                        ...
                        {
                            managerHandleMouseClick(slot2, slot2.slotNumber, state, ClickType.QUICK_MOVE);
                        }
                    }
                }
            }
            ...
            {
                managerHandleMouseClick(slot, k, state, ClickType.PICKUP_ALL);
            }
        ...
        else
        {
            ...
            if (this.clickedSlot != null && this.mc.gameSettings.touchscreen)
            {
                if (state == 0 || state == 1)
                {
                    ...
                    {
                        managerHandleMouseClick(this.clickedSlot, this.clickedSlot.slotNumber, state, ClickType.PICKUP);
                        managerHandleMouseClick(slot, k, 0, ClickType.PICKUP);
            ...
            {
                managerHandleMouseClick((Slot)null, -999, Container.func_94534_d(0, this.dragSplittingLimit), ClickType.QUICK_CRAFT);

                for (Slot slot1 : this.dragSplittingSlots)
                {
                    managerHandleMouseClick(slot1, slot1.slotNumber, Container.func_94534_d(1, this.dragSplittingLimit), ClickType.QUICK_CRAFT);
                }

                managerHandleMouseClick((Slot)null, -999, Container.func_94534_d(2, this.dragSplittingLimit), ClickType.QUICK_CRAFT);
            }
            else if (this.mc.thePlayer.inventory.getItemStack() != null)
            {
                if (state == this.mc.gameSettings.keyBindPickBlock.getKeyCode() + 100)
                {
                    managerHandleMouseClick(slot, k, state, ClickType.CLONE);
                }
                else
                {
                    ...
                    managerHandleMouseClick(slot, k, state, flag1 ? ClickType.QUICK_MOVE : ClickType.PICKUP);
                }
            }
            else if (state >= 0)
    ...
    protected void handleMouseClick(Slot slotIn, int slotId, int mouseButton, ClickType type)
    {
        ...
        //this.mc.playerController.func_187098_a(this.inventorySlots.windowId, slotId, mouseButton, type, this.mc.thePlayer);
        manager.handleSlotClick(slotId, mouseButton, type);
    }
    ...
    protected boolean checkHotbarKeys(int keyCode)
    {
        if (this.mc.thePlayer.inventory.getItemStack() == null && this.theSlot != null)
        {
            for (int i = 0; i < 9; ++i)
            {
                ...
                {
                    managerHandleMouseClick(this.theSlot, this.theSlot.slotNumber, i, ClickType.SWAP);
                    return true;
                }
            }
        }
    ...
    public void managerHandleMouseClick(Slot slot, int slotIndex, int button, ClickType modifiers){
        manager.handleMouseClick(slot, slotIndex, button, modifiers);
    }

    /* ============================ END NEI GENERATED METHODS ===========================*/
